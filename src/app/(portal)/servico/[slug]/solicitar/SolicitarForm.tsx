...
export default function SolicitarForm({ slug, nome }: { slug: string; nome: string }) {
  ...
  return (
    <form onSubmit={enviar} className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
      ...
      <button disabled={enviando} className="w-full px-5 py-3 rounded-xl bg-blue-600 text-white font-bold text-sm hover:bg-blue-700 disabled:opacity-60"><i className="fas fa-paper-plane mr-2" />{enviando ? "Enviando…" : "Enviar solicitação"}</button>
