...
import { useFiscalCaixaPostal } from "./use-fiscal-caixa-postal";
...
describe("useFiscalCaixaPostal", () => {
  it("busca as mensagens da caixa postal com sucesso", async () => {
    const dados = { items: [{ id: "1", assunto: "Notificação" }] };
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(jsonResponse(200, dados)));

    const { result } = renderHook(() => useFiscalCaixaPostal(), { wrapper });
